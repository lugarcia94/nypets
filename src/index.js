...
if(config.minicart.actived) {
    let rootMinicart = Array.from(document.querySelectorAll('[data-minicart]'));

    if (rootMinicart.length) {
        rootMinicart.forEach((root) => {
            ReactDOM.render(
                <Provider store={store}>
                    <Minicart id={root.getAttribute('data-minicart')} />
                </Provider>,
                root
            );
        })
